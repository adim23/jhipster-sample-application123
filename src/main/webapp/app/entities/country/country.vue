<template>
  <div>
    <h2 id="page-heading" data-cy="CountryHeading">
      <span v-text="$t('jhipsterSampleApplication123App.country.home.title')" id="country-heading">Countries</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('jhipsterSampleApplication123App.country.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CountryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('jhipsterSampleApplication123App.country.home.createLabel')"> Create a new Country </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && countries && countries.length === 0">
      <span v-text="$t('jhipsterSampleApplication123App.country.home.notFound')">No countries found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('countryName')">
              <span v-text="$t('jhipsterSampleApplication123App.country.countryName')">Country Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'countryName'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('region.regionName')">
              <span v-text="$t('jhipsterSampleApplication123App.country.region')">Region</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'region.regionName'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
          ...
          <tr v-for="country in countries" :key="country.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="country.region">
                <router-link :to="{ name: 'RegionView', params: { regionId: country.region.id } }">{{
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="jhipsterSampleApplication123App.country.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-country-heading" v-text="$t('jhipsterSampleApplication123App.country.delete.question', { id: removeId })">
