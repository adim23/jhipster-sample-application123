<template>
  <div>
    <h2 id="page-heading" data-cy="RegionHeading">
      <span v-text="$t('jhipsterSampleApplication123App.region.home.title')" id="region-heading">Regions</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('jhipsterSampleApplication123App.region.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'RegionCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('jhipsterSampleApplication123App.region.home.createLabel')"> Create a new Region </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && regions && regions.length === 0">
      <span v-text="$t('jhipsterSampleApplication123App.region.home.notFound')">No regions found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('regionName')">
              <span v-text="$t('jhipsterSampleApplication123App.region.regionName')">Region Name</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="jhipsterSampleApplication123App.region.delete.question"
          data-cy="regionDeleteDialogHeading"
          v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-region-heading" v-text="$t('jhipsterSampleApplication123App.region.delete.question', { id: removeId })">
